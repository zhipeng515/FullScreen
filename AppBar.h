...
#include <map>
#include "../CommonFunction/ThreadTimer.h"

#define WM_APPBAR_THREAD_TIMER		WM_USER+123
template <class T>
unsigned WINAPI AppBar_Timer_Thread(void* lpParam);

using namespace ATL;
template <class T>
class CAppBar : public ThreadTimer<T>
{
public: //constants
	enum DockingSide
	{
...
private:
	WTL::CRect	m_rcOriginal;	//window rect while not docking on any side
	WTL::CRect	m_rcStartDraging;
	WTL::CRect	m_rcLastDraging;
	WTL::CPoint	m_ptStartDraging;
	bool	m_bDraging;
	...
	int		m_nSlideBeginTick;
	CSize	m_szSlideSize;
	CPoint	m_ptSlideBegin;
...
public:
	...
	BEGIN_MSG_MAP(CAppBar)
		...
		MESSAGE_HANDLER(WM_THREADTIMER, OnThreadTimer)
...
public:
	...
	void InitAppBar(UINT uFlags=APPBAR_DOCKING_ALL, bool bAutoHide=false, bool bKeepSize=false, bool bCanDrag=false)
	{
	...
	void DockAppBar(UINT uFlag, int showTime = 500)
	{
		T* pT = static_cast<T*>(this);
		...
		else
		{
			...
			if (m_bAutoHide)
			{
				m_uHidingTimer = pT->SetThreadTimer(m_uHidingTimer, showTime);
			}
		}
	}
	...
	LRESULT OnNcHitTest(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/)
	{
		...
		{
			m_uShowingTimer = pT->SetThreadTimer(m_uShowingTimer, 500);
		}
		...
		switch (state)
		{
			...
			case WA_CLICKACTIVE:
				...
				if (m_uCurrentDockingSide!=APPBAR_DOCKING_NONE && m_bHiding && m_bAutoHide)
				{
					...
					m_uHidingTimer = pT->SetThreadTimer(m_uHidingTimer, 500);
				}
				break;
	...
	void DoOnTimer(UINT nTimerID)
	{
		T* pT = static_cast<T*>(this);
		if (nTimerID == m_uSlideTimer)
		{
			if (SlideWindow(!m_bHiding))
			{
				pT->KillThreadTimer(nTimerID);
			}
		}
		else if (nTimerID == m_uHidingTimer)
		{
			...
			else
			{
				...
				CWindow wndOwner = wndActive.GetWindow(GW_OWNER);
				if ((!rcWnd.PtInRect(pt)) && (wndActive.m_hWnd != pT->m_hWnd) &&
					(wndActive.m_hWnd != NULL) && wndOwner.m_hWnd != pT->m_hWnd)
				{
					pT->KillThreadTimer(nTimerID);
					DoHideAppBar();
				}
		...
		else if (nTimerID == m_uShowingTimer)
		{
			pT->KillThreadTimer(nTimerID);
	...
	LRESULT OnThreadTimer(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& bHandled)
	{
		//enable derived class to handle this message
		bHandled = FALSE;
...
protected:
	...
	void DrawFrame(WTL::CRect rc)
	{
		...
		WTL::CWindowDC dc(::GetDesktopWindow());
		WTL::CBrush brush = WTL::CDCHandle::GetHalftoneBrush();
	...
	void BeginSlide(bool bShow)
	{
		...
		m_szSlideSize.SetSize(dx, dy);
		m_ptSlideBegin = rcWnd.TopLeft();
		m_nSlideBeginTick = GetTickCount();

		pT->SetThreadTimer(m_uSlideTimer, 33);
	...
	bool SlideWindow(bool bShow)
	{
		T* pT = static_cast<T*>(this);

		int x, y, totalTime, dt;
		totalTime = bShow ? SHOW_TICKS : HIDE_TICKS;
		dt = GetTickCount();

		x = m_ptSlideBegin.x + m_szSlideSize.cx * (dt - m_nSlideBeginTick) / totalTime;
		y = m_ptSlideBegin.y + m_szSlideSize.cy * (dt - m_nSlideBeginTick) / totalTime;

		pT->SetWindowPos(NULL, x, y, 0, 0,
					SWP_NOSIZE | SWP_NOZORDER | SWP_DRAWFRAME | SWP_NOACTIVATE);

		if (dt >= m_nSlideBeginTick + totalTime)
		{
			//Ajust position to the right place
			pT->SetWindowPos(NULL, m_ptSlideBegin.x + m_szSlideSize.cx,
				m_ptSlideBegin.y + m_szSlideSize.cy,
				0, 0,
